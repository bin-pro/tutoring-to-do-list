...
const ToDoItem = ({ toDoItemDataList, setToDoItemDataList }) => {
  ...
  const onClickDeleteToDoItemData = (id) => () => {
    setToDoItemDataList((prevState) => {
      ...
      return updatedList.map((listItem, index) => {
        if (listItem.id > id) {
          return {
            ...listItem,
            id: listItem.id - 1,
  ...
  return toDoItemDataList.map((toDoItemData) => {
    var toggle = toDoItemData.isChecked ? "checked" : "";

    const listContent = () => {
      const content = toDoItemData.content;

      if (!toDoItemData.isUpdating) {
        return (
          <li key={`todo_item_${toDoItemData.id}`} className={`${toggle}`}>
            {content}
          </li>
        );
      }
